    ...
    ### Core Game Features
    - **Game Board Display**: 10×20 grid with proper brick rendering and collision detection
    - **Brick Movement**: Left/Right arrow keys for horizontal movement with boundary checking
    - **Brick Rotation**: Up arrow or `Z` key to rotate pieces with wall-kick collision detection
    ...
    ### User Interface
    - **Main Menu**: Navigate between Start Game, Settings, and Exit options
    - **Settings Screen**: 
    ...
    |--------|---------|-------------|
    | Move Left | Left Arrow | Move brick left with boundary checking |
    | Move Right | Right Arrow | Move brick right with boundary checking |
    | Rotate | Up Arrow or Z | Rotate brick 90° clockwise with wall-kick |
    | Soft Drop | Space (single tap) | Controlled descent at increased speed |
    | Hard Drop | Space (double tap) | Instant placement at bottom |
    | Hold Piece | V | Swap current brick with held piece (once per turn) |
    | Pause/Resume | Escape | Pause game without losing state |
    ...
    **None** - All implemented features are functioning as expected.
    ...
    - **Background Music (BGM)**: Sound effects are fully implemented; a looping background track was not added due to scope constraints and potential audio licensing considerations
    - **Custom Themes/Skins**: The UI uses a fixed dark theme with static colors; custom color schemes and visual themes were not implemented
    - **Power-ups/Abilities**: Special gameplay modifiers (slow-down, block freezing, etc.) were not included to maintain classic Tetris gameplay
    - **Player Profiles/Accounts**: No user account system; only local high score files per difficulty
    - **Accessibility Color Modes**: High-contrast or colorblind-friendly palettes not provided
    - **Network Multiplayer**: Online gameplay or leaderboards not implemented
    - **Game Replays**: No replay recording or playback functionality
    ...
    **Resolution**: Added `synchronized` keyword to `GameSettings.saveSettings()` method. Implemented atomic write pattern using temporary file + rename (write to `settings.txt.tmp`, then rename to `settings.txt`). Prevents partial file corruption on interruption.
    ...
    ├── GameConfig.java                 # Global configuration constants
    ├── controller/
    │   ├── EventSource.java            # Enum: USER, THREAD
    │   ├── EventType.java              # Enum: MOVE, ROTATE, DROP, HOLD, etc.
    │   ├── GameController.java         # Main game logic & state management
    │   ├── InputHandler.java           # Keyboard input processing
    │   ├── MoveEvent.java              # Event object for input
    ├── model/
    │   ├── Board.java                  # Interface defining board contract
    │   ├── SimpleBoard.java            # Main board implementation
    │   ├── BrickRotator.java           # Brick rotation with wall-kick
    │   ├── ClearRow.java               # Line clearing logic
    │   ├── Difficulty.java             # Enum: EASY, NORMAL, HARD, EXTRA
    │   ├── DownData.java               # DTO for movement results
    │   ├── GameSettings.java           # Persistent user settings
    │   ├── HighScoreManager.java       # High score persistence per difficulty
    │   ├── MatrixOperations.java       # Grid utility methods
    │   ├── NextShapeInfo.java          # DTO for brick preview data
    │   ├── Score.java                  # Score state with property binding
    │   ├── ViewData.java               # DTO for brick state transfer
    │   └── bricks/
    │       ├── IBrick.java             # Interface for brick types
    │       ├── Brick.java              # Abstract base class
    │       ├── JBrick.java             # J-piece implementation
    │       ├── LBrick.java             # L-piece implementation
    │       ├── OBrick.java             # O-piece (square) implementation
    │       ├── SBrick.java             # S-piece implementation
    │       ├── TBrick.java             # T-piece implementation
    │       ├── ZBrick.java             # Z-piece implementation
    │       ├── BrickGenerator.java     # Interface for brick generation
    │       └── RandomBrickGenerator.java # 7-bag randomization with queue
    ├── view/
    │   ├── Main.java                   # Application entry point
    │   ├── GuiController.java          # Main game view controller
    │   ├── MainMenuController.java     # Menu navigation
    │   ├── SettingController.java      # Settings UI
    │   ├── GameOverPanel.java          # Game over display
    │   ├── NotificationPanel.java      # Transient notifications
    │   └── InputEventListener.java     # Interface for input events
    ```
    ...
    │   ├── game_layout.fxml            # Game board layout
    │   └── settings_screen.fxml        # Settings layout
    ├── css/
    │   ├── global.css                  # Global styles
    │   ├── main_menu.css               # Menu styles
    │   ├── game_layout.css             # Game styles
    │   └── settings_screen.css         # Settings styles
    ...
    **game controls, line clearing, pause/resume and collision detection** - https://github.com/Hasnatrasool163/Tetris-in-Java
    ...
    - **CSS Stylesheets**: 4
    - **Unit Tests**: 4
    - **Lines of Code**: ~3,500+ (model, controller, view)
    - **Configuration Files**: `pom.xml`, `settings.txt`, `highscore_*.txt`
